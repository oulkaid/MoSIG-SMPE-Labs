...
summary(reg2)
```
- Now, we clearly notice an improvement of the relevance of the model ; the coefficient of determination is much more significant ($\hat \beta_2$) is `14.5099` (was `8.9999` for `reg1`). This goes along with a decrease of the intercept ($\hat \beta_1$).
- This, our assumption was valid ; removing the outlier from the linear regression model is helpful. 
...
```
The purpose of this function is to start from a linear regression model (`reg0` is this case) and perform some analysis by adding at each time a new variable. It seems that the variables are added in an order such that the variable with the highest `p-value` is added first, and so on.
...
- If you consider this definition, we can assume that the multivariate model (`BRW~AUD + MOB + HIP`) fits better.
...
- Conclusion : there is no correlation between the auditory brain volume and diet ; the sum of squared residuals is way more large than the sum of squares of the model. In addition, the corresponding `p-value` is high, which means that there is no clear correlation linking the auditory brain volume and diet.
